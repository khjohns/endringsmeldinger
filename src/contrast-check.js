...
const usages = [
	...
	{ fg: 'ink-muted',     bg: 'canvas',      size: 10, weight: 600, label: 'Seksjonstittel på canvas (.section-label)' },
	{ fg: 'ink-muted',     bg: 'felt',        size: 10, weight: 600, label: 'Seksjonstittel på felt (.section-label)' },
	...
	{ fg: 'ink-secondary', bg: 'canvas',      size: 14, weight: 400, label: 'Begrunnelse-brødtekst i historikk' },
...
function analyzeTheme(themeName, theme) {
	...
	console.log(
		'Farge'.padEnd(16) +
		solidSurfaces.map(s => s.padEnd(13)).join('') +
		'  Hex'
	);
	...
	for (const [inkName, inkHex] of Object.entries(allColors)) {
		...
		for (const sName of solidSurfaces) {
			...
			if (!bgHex) { line += '  n/a'.padEnd(13); continue; }
