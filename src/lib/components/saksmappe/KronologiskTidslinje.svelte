  ...
  const futureNodes = $derived.by(() => {
    ...
    for (const track of tracks) {
      ...
      if (isAwaitingResponse(track.status)) {
        ...
        const latestEvent = allNodes
          .filter((n) => n.spor === track.spor && n.actor === 'TE')
          .at(-1);
          ...
          >
            ...
            <div class="node-side node-side-venstre">
              {#if isLeft}
                ...
                <div
                  class="node-info node-info-venstre"
                  onclick={() => handleNodeClick(node.id, node.event)}
                >
                  <div
                    class="bar bar-venstre"
                    class:bar-dashed={node.isBH}
                    style="width: {barWidth}px; {node.isBH
                      ? `border-color: ${node.sporColor}`
                      : `background-color: ${node.sporColor}`}"
                  ></div>
                  <div class="info-tekst info-tekst-right">
                    <span class="aktor-badge" class:aktor-bh={node.isBH}>{node.actorName}</span>
                    <span class="verdi-tekst" class:verdi-aktiv={entangled || isActive}
                      >{node.valueLabel ?? ''}</span
                    >
                    <span class="dok-ref"
                      >{node.label}{#if node.revision}
                        (Rev {node.revision}){/if}</span
                    >
                  </div>
                </div>
              {/if}
            ...
            <div class="node-side node-side-hoyre">
              {#if isRight}
                <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
                <div
                  class="node-info node-info-hoyre"
                  onclick={() => handleNodeClick(node.id, node.event)}
                >
                  <div
                    class="bar bar-hoyre"
              ...
              {#if isCenter}
                <!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
                <div
                  class="node-info node-info-hoyre node-info-center"
                  onclick={() => handleNodeClick(node.id, node.event)}
                >
                  <div class="info-tekst">
                    <span class="aktor-badge" class:aktor-bh={node.isBH}>{node.actorName}</span>
            ...
            <div class="node-side node-side-venstre">
              {#if isLeft}
                ...
                <div class="node-info node-info-venstre" onclick={() => handleNodeClick(fn.id)}>
                  <div
                    class="bar bar-venstre bar-dashed"
                    style="width: 48px; border-color: {fn.sporColor}"
                  ></div>
                  <div class="info-tekst info-tekst-right">
                    <span class="aktor-badge aktor-bh">{fn.actorName}</span>
                    <span class="verdi-tekst verdi-muted" class:verdi-aktiv={entangled}
                      >Forventet svar</span
                    >
            ...
            <div class="node-side node-side-hoyre">
              {#if isRight}
                ...
                <div class="node-info node-info-hoyre" onclick={() => handleNodeClick(fn.id)}>
                  <div
                    class="bar bar-hoyre bar-dashed"
                    style="width: 48px; border-color: {fn.sporColor}"
                  ></div>
                  <div class="info-tekst">
                    <span class="aktor-badge aktor-bh">{fn.actorName}</span>
