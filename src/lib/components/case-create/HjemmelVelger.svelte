<script lang="ts">
  ...
  function erValgt(forholdKode: string, hjemmelKode: string | null): boolean {
    ...
    return (
      valgt.kontraktsforhold.kode === forholdKode && (valgt.hjemmel?.kode ?? null) === hjemmelKode
    );
  }
  ...
  const valgtBeskrivelse = $derived(
    valgt?.hjemmel?.beskrivelse ?? valgt?.kontraktsforhold.beskrivelse ?? ''
  );
</script>
