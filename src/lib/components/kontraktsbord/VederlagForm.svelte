<script lang="ts">
  ...
  const previousEvent = store.isDemo
    ? undefined
    : submissionRefs(store.timeline, 'vederlag').response;
  const previous = previousEvent?.data as ResponsVederlagEventData | undefined;
    ...
    lastResponseEvent:
      previous && previousEvent
        ? {
            ...
            hovedkravVarsletITide: previous.hovedkrav_varslet_i_tide,
            riggVarsletITide: previous.rigg_varslet_i_tide,
            produktivitetVarsletITide: previous.produktivitet_varslet_i_tide,
            akseptererMetode: previous.aksepterer_metode,
            oensketMetode: previous.oensket_metode,
            hovedkravVurdering: previous.hovedkrav_vurdering,
            hovedkravGodkjentBelop: previous.hovedkrav_godkjent_belop,
            riggVurdering: previous.rigg_vurdering,
            riggGodkjentBelop: previous.rigg_godkjent_belop,
            produktivitetVurdering: previous.produktivitet_vurdering,
            produktivitetGodkjentBelop: previous.produktivitet_godkjent_belop,
          }
        : undefined,
  });
  ...
  const draft = createFormDraft(
    ...
    (saved) => {
      ...
      akseptererMetode = saved.akseptererMetode;
      oensketMetode = saved.oensketMetode;
      hovedkravVurdering = saved.hovedkravVurdering;
      hovedkravGodkjentBelop = saved.hovedkravGodkjentBelop;
      riggVurdering = saved.riggVurdering;
      riggGodkjentBelop = saved.riggGodkjentBelop;
          ...
          void submission.run(async () => {
            ...
            if (store.isDemo) {
              store.sendVederlagSvar(computed.totalGodkjent, {
                ...
                produktivitetVurdering,
                produktivitetGodkjentBelop,
                subsidiaerGodkjentBelop: isSubsidiaer
              ...
              const { eventType, data } = buildEventData(
                ...
                computed,
                {
                  vederlagKravId: requireEventId(refs.claimId),
                  lastResponseEventId: refs.responseId,
                  isUpdateMode: Boolean(refs.responseId),
                },
                '',
              ...
              await store.submit(eventType as EventType, data);
            }
          }, onsend);
      },
    });
      ...
      <div class="sammendrag-okonomi">
        {#if domainConfig.metode}
          <div class="sammendrag-metode">
            ...
            <span class="sammendrag-metode-verdi"
        ...
        <div class="sammendrag-kravlinjer">
          ...
          {#if domainConfig.harRiggKrav && domainConfig.riggBelop}
            <div class="sammendrag-kravlinje">
              ...
              <span class="font-mono sammendrag-kravlinje-belop"
                >{fmt(domainConfig.riggBelop)},-</span
              >
            </div>
          {/if}
          {#if domainConfig.harProduktivitetKrav && domainConfig.produktivitetBelop}
        ...
        <div class="result-box konsekvens-{resultat.variant}">
          ...
          <div class="result-tabell">
            ...
            <div class="tr tr-total">
              ...
              {#if computed.visSubsidiaertResultat}
                <span class="td td-tal font-mono"
                  >{fmt(computed.totalGodkjentInklPrekludert)} kr</span
                >
              {/if}
            </div>
          </div>
