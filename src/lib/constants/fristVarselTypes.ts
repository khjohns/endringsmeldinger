...
export function getFristVarseltypeValues(): [string, ...string[]] {
  const values = FRIST_VARSELTYPE_OPTIONS.filter((opt) => opt.value !== '').map((opt) => opt.value);
...
export const FRIST_VARSELTYPE_DESCRIPTIONS: Record<string, string> = {
  varsel:
    'Varsel sendes «uten ugrunnet opphold», selv om du ennå ikke kan fremsette et spesifisert krav (§33.4).',
  spesifisert:
    'Når du har grunnlag for å beregne omfanget, skal du angi og begrunne antall dager (§33.6.1).',
  begrunnelse_utsatt:
    'Begrunn hvorfor grunnlaget for å beregne kravet ikke foreligger (§33.6.2 b). Bestemmelsen i §33.6.1 gjelder videre.',
};
