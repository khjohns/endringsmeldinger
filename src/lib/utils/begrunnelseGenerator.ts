...
import { lockedValueFormatters } from './lockedValueTokens';
...
/**
 * Options for begrunnelse generation
 */
...
/**
 * Format currency, optionally as a locked token
 */
function formatCurrency(amount: number, useTokens = false): string {
  if (useTokens) {
...
/**
 * Format days, optionally as a locked token
 */
function formatDager(dager: number, useTokens = false): string {
  if (useTokens) {
...
/**
 * Format percentage, optionally as a locked token
 */
function formatProsent(prosent: number, useTokens = false): string {
  if (useTokens) {
 ...
 * @param options - Generator options (useTokens for locked values)
 */
...
/**
 * Generisk funksjon for særskilte krav (rigg/drift eller produktivitet) i forsering-kontekst
 */
function generateForseringSaerskiltKravSection(
  ...
  const kravLabelKort = kravType === 'rigg' ? 'rigg- og driftskostnader' : 'produktivitetstap';
...
/** Generate section for rigg/drift krav i forsering-kontekst */
...
/** Generate section for produktivitetstap krav i forsering-kontekst */
...
/**
 * Generisk funksjon for subsidiær tekst for særskilte krav i forsering-kontekst
 * Brukes når TE ikke har forseringsrett (alle avslag var berettiget)
 */
