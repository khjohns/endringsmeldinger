<script lang="ts">
	...
	const timelineData = $derived.by(() => {
		...
		if (!timeline) return {
			originalEventId: '',
			teBegrunnelseHtml: '',
			bhSvar: null as { resultat: GrunnlagResponsResultat; varsletITide?: boolean; begrunnelseHtml: string; dato?: string } | null,
			tidligereSvar: [] as { rolle: 'TE' | 'BH'; versjon: number; html: string; dato?: string; resultat?: string }[],
		};
		...
		const latestGrunnlag = grunnlagEvents.length > 0 ? grunnlagEvents[grunnlagEvents.length - 1] : null;
		const latestGrunnlagData = latestGrunnlag?.data as unknown as Record<string, unknown> | undefined;

		// BH's latest response
		const latestResponse = responsEvents.length > 0 ? responsEvents[responsEvents.length - 1] : null;
		const latestResponseData = latestResponse?.data as unknown as Record<string, unknown> | undefined;
		...
		return {
			...
			bhSvar: state?.grunnlag.bh_resultat != null ? {
