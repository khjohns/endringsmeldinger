...
### 1.1 Repo-strategi — BESLUTTET: Nytt repo

**Valg A: Nytt repo.** Begrunnelse:
- Helt annet domene (KOE/NS 8407 vs. offentlig anskaffelse) — delt repo gir falsk kobling
- Ren start med Vite 7 + `uv` + Svelte 5.49 uten å arve anti-mønstre fra eksisterende kode
- Det gjenbrukbare fra procurement-api er designtokens og dokumentasjon (allerede dekket av §2.2), ikke kode

**Konsekvenser:**
- Domain-pakken kopieres direkte (se §2.1) — ingen npm workspace eller git submodule nødvendig
- Backend (Flask) lever i unified-timeline-repoet som før — API-only kommunikasjon
- Separat CI/CD settes opp i Fase 0
...
### 2.4 Backend (forbedringer, ikke funksjonelle endringer)

Backend kjører som før. Svelte-appen snakker til samme Flask API. Forbedring: **migrer til `uv`** for Python-avhengigheter — gir 10-100x raskere installs, innebygd lockfile (`uv.lock`), og reproduserbare Docker-builds. Drop-in erstatning for pip/pip-tools.
...
| Bundler | Vite 7 | Krever Node 20.19+. Vite 6 har kjente CVE-er (path traversal, DNS rebinding). Rolldown opt-in tilgjengelig |
...
| Rich text editor | Tipex (`@friendofsvelte/tipex`) | Svelte 5-native Tiptap-wrapper med floating toolbar. Testet i procurement-api |
...
| Python deps | `uv` | Erstatter pip/pip-tools. 10-100x raskere, innebygd lockfile, reproduserbare Docker-builds |
...
| **`createContext`** (5.40) | Erstatter module-level singletons for stores — vurder som alternativ | `svelte.dev/docs/svelte/context` |
| **Writable `$derived`** (5.25) | Toveisflyt i evaluering uten ekstra `$state` | `svelte.dev/docs/svelte/$derived` |
| **`$state.eager`** (5.41) | Ytelseskritisk state (scoring-matrise) — skippar proxy | `svelte.dev/docs/svelte/$state` |
| **`fork` API** (5.42) | Isolert state-forking for testing og SSR | `svelte.dev/docs/svelte/$state` |
...
### 3.2 Tailwind v4-gotchas for Svelte

- **`@theme inline`** kreves for tokens som refererer runtime `var()`-verdier (f.eks. dark mode-switching). Vanlig `@theme` resolver verdier ved build-tid og mister runtime-referansene
- **`@apply` i Svelte `<style>`-blokker** krever `@reference "tailwindcss"` som prefix — uten dette feiler `@apply` stille

### 3.3 SvelteKit-spesifikke mønstre
...
### 3.4 URL-struktur og ruting

```
src/routes/
├── [prosjektId]/
│   ├── +page.svelte                 # Saksliste (minimal)
│   ├── [sakId]/
│   │   ├── +page.svelte             # CasePage / Forhandlingsbordet
│   │   └── [spor]/
│   │       └── +page.svelte         # Spordetalj (grunnlag/vederlag/frist)
│   └── +layout.svelte               # Prosjekt-kontekst (X-Project-ID header)
└── +layout.svelte                    # App shell, auth guard
```

Prosjekt-ID i URL-en gir multi-prosjekt-støtte fra dag 1. `[prosjektId]/+layout.svelte` setter `X-Project-ID` header i API-klienten.
...
- [x] ~~Avklar repo-strategi~~ → Nytt repo (besluttet)
- [ ] Opprett nytt repo og initialiser SvelteKit-prosjekt (`npx sv create`)
...
- [ ] Verifiser at Vite 7 brukes (ikke 6) — sjekk `package.json` etter `sv create`
- [ ] Evaluer `createContext` (Svelte 5.40) vs module-level singletons for store-arkitektur
- [ ] Konfigurer `@theme inline` i Tailwind for runtime CSS-variable-tokens
- [ ] Migrer Python-backend til `uv` (`uv init`, `uv.lock`, oppdater Dockerfile)
- [ ] Sett opp Cloud Run-deployment (Dockerfile, `deploy-web.sh`, Cloud Build) — basert på procurement-api-oppsettet
...
| LockedValueToken | Ikke-redigerbare verdier i begrunnelse | Tipex custom extension |
...
### Fase 2: Saksliste + Forhandlingsbordet (oversiktsside)
**Mål:** Minimal saksliste + landingsside for én KOE-sak med tre sporkort + tidslinje.

**Saksliste** (`/[prosjektId]`):
- Minimal liste over saker i prosjektet
- TanStack Query mot eksisterende API
- Navigasjon til CasePage

**Forhandlingsbordet** (`/[prosjektId]/[sakId]`), basert på `koe-forhandlingsbord.md`:
...
- Navigasjon til spordetalj (`/[prosjektId]/[sakId]/grunnlag` etc.)
...
- Bruk onMount eller +page.ts load for data-henting, IKKE $effect
  ($effect for data-fetching er et kjent anti-mønster — se arkitektur-review §11.3)
- Vurder createContext (5.40) for store-deling fremfor module-level singletons
...
|--------|---------------|-----------|------------|
| Tipex ustabil eller mangler features | Lav | Begrunnelsespanelet fungerer dårlig | Allerede testet i procurement-api. Fallback: vanilla Tiptap med manuell Svelte-binding |
...
| Fase | Kriterium |
|------|-----------|
| 0 | Domenetester grønne. Tailwind fungerer med designtokens. Dev server kjører. Cloud Run deploy fungerer. |
| 1 | Alle 16 primitiver implementert med tester og a11y. Storybook/showcase-side. |
| 2 | Saksliste viser saker. Forhandlingsbordet viser reelle data fra API. Navigasjon til spordetalj fungerer. |
...
1. ~~**Repo-strategi**~~ → **BESLUTTET:** Nytt repo (se §1.1)
2. ~~**Domain-pakke**~~ → **BESLUTTET:** Direkte kopi (følger av nytt repo — ingen deling nødvendig)
3. ~~**Dark mode**~~ → **BESLUTTET:** Kun dark theme fra start. Analysebordet er designet mørkt — light mode vurderes i Fase 7 ved behov
4. ~~**Deployment**~~ → **BESLUTTET:** Cloud Run, samme oppsett som procurement-api (multi-stage Docker, Gunicorn, GCP Secret Manager). Robust, god gratis kvote, kjent infrastruktur
5. ~~**Saksliste**~~ → **BESLUTTET:** Minimal saksliste i Fase 2. URL-struktur: `/[prosjektId]/[sakId]` (CasePage) med saksliste på `/[prosjektId]`. Gir multi-prosjekt-støtte i rutingen fra dag 1
6. ~~**Tiptap Svelte-wrapper**~~ → **BESLUTTET:** Bruk Tipex (`@friendofsvelte/tipex`) — Svelte 5-native, floating toolbar, allerede testet i procurement-api. Erstatter Tiptap+svelte-tiptap i teknologistabellen
