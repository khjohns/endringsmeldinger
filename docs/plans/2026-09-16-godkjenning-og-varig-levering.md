...
| Godkjenning av ansvarsgrunnlaget (GFK-06): fullmakten regnes av TEs krevde beløp, vederlag pluss krevde fristdager ganget med dagmulktssatsen. Er kravet ikke tallfestet, kreves hele kjeden | 23.09, oppdragsgiver | Bygget 24.09 (merknaden i GFK-06). Mangler dagmulktssatsen, gjelder B-06 |
| GFK-06 presisert: ansvar har ingen egen verdi, fullmakten regnes per spor. Et spor som besvares i samme brev, har verdien i svaret; ellers gjelder TEs krav. Et krav som ikke er sendt, krever hele kjeden som et krav som ikke er tallfestet; et trukket krav er 0. Avslått og frafalt ansvar er 0 | 24.09, oppdragsgiver | Bygget 24.09. Ved «frafalt» (§ 32.3 c) må byggherren utstede en endringsordre for det utførte arbeidet, med egen fullmakt; varsel eller tom EO-sak i frontenden er ikke bygget |
| Ubegrenset fullmakt kan sende alene når beløpet ikke kan verdsettes | 23.09, oppdragsgiver | Bygget 24.09 i `resolve_route` og speilet `resolveRoute` (merknaden i GFK-02). Gjelder fristsvar med ny sluttdato (GFK-02) og endringsordrer (`resolve_route`). Alle andre trenger fortsatt hele kjeden, og kjeden må dekke det som lar seg verdsette |
...
| GFK-02 | Lukket | `exposure()` ignorerte `ny_sluttdato`. **Merknad 2026-09-23:** rettet i `approval_route`: en ny sluttdato i et fristsvar gir `amount=None`, krever hele kjeden, og kjeden må dekke det som lar seg verdsette (`minimum`), som for endringsordrer. Uten dagmulktssats er atferden uendret (B-06); testen påstår bare at ruten aldri blir kortere enn kjeden. Frontendens `calculateAuthority` speiler ikke dette, men skjemaet sender ikke `ny_sluttdato`. **Avgjort 23.09** (3.1): ubegrenset fullmakt skal kunne sende alene. **Merknad 2026-09-24:** bygget. En avsender med ubegrenset fullmakt får tom rute også når beløpet ikke kan verdsettes, for fristsvar og endringsordrer, og også uten konfigurert kjede. Alle andre trenger fortsatt hele kjeden. Unntatt er en endringsordre med fristdager uten dagmulktssats: der gjelder B-06, og kjeden kreves som før. Pakker i omløp med ubegrenset avsender blir returnert ved neste avstemming, fordi ruta endres. `test_ny_sluttdato_kan_ikke_godkjennes_av_saksbehandler_alene` påstod det motsatte og er snudd etter vedtaket, som `test_ny_sluttdato_kan_sendes_alene_med_ubegrenset_fullmakt` | K 23.09 | — |
...
| GFK-06 | Lukket | Godkjent grunnlag alene verdsettes til 0 kr. **Merknad 2026-09-23:** reprodusert gjennom `ApprovalService`: en prosjektleder godkjenner ansvaret for et krav på 50 mill. alene, og pakken godkjennes ved innsending. Ikke rettet: hvordan en slik godkjenning skal verdsettes, er en fullmaktsbeslutning for byggherren. **Avgjort 23.09** (3.1): TEs krevde beløp, og hele kjeden når kravet ikke er tallfestet. **Merknad 2026-09-24:** rettet etter vedtaket og presiseringen samme dag (3.1). `exposure` verdsetter godkjent ansvar per spor av TEs krav, som `ApprovalService` leser av saken (`krav_fra_tilstand`), og setter i pakkens `authorityContext.krav`. Krav som ikke er sendt eller tallfestet, gir hele kjeden med det verdsatte som gulv. Endrer TE kravet etter at pakken er laget, returneres pakken. Krevde dager uten sats avvises som for et fristsvar (B-06 uendret). Frontendens `calculateAuthority` og `kravFraSak` speiler regelen. Reproduksjonen er ordinær test med strengere påstander; tre ordinære tester som godkjente ansvar uten krav, har fått trukne eller tallfestede krav i oppsettet, og to tellinger av hendelser er gjort relative | K 24.09 | — |
