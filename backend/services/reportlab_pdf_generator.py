...
class ReportLabPdfGenerator:
    ...
    def generate_pdf(
        ...
        saksbehandler: SignatureInfo | None = None,
        godkjenner: SignatureInfo | None = None,
    ) -> bytes | None:
        """
        ...
        try:
            ...
            story.append(
                Paragraph(
                    title_map.get(sakstype, "Krav om Endringsordre"),
                    self.styles["KoeTitle"],
                )
            )
            story.append(
                HRFlowable(
    ...
    def _build_grunnlag_section(
        ...
        if grunnlag.status == "ikke_relevant":
            elements.append(
                Paragraph(
                    "<i>Ikke relevant for denne saken</i>", self.styles["KoeBodyText"]
                )
            )
            return elements
        ...
        elements.append(
            Paragraph("<b>ENTREPRENØR KREVER:</b>", self.styles["KoeSubHeader"])
        )
        ...
        if grunnlag.underkategori and grunnlag.hovedkategori:
            underkat_label = get_underkategori_label(
        ...
        if grunnlag.bh_resultat:
            ...
            elements.append(
                Paragraph("<b>BYGGHERRE SVARER:</b>", self.styles["KoeSubHeader"])
            )

            bh_data = []
            resultat_label = self.GRUNNLAG_RESULTAT_MAP.get(
                grunnlag.bh_resultat, grunnlag.bh_resultat
            )
                ...
                TableStyle(
                    [
                        ...
                        (
                            ...
                            (0, 0),
                            (0, -1),
                            colors.HexColor(self.COLORS["muted"]),
                        ),
                        ("VALIGN", (0, 0), (-1, -1), "TOP"),
                        ("BOTTOMPADDING", (0, 0), (-1, -1), 4),
                    ]
    ...
    def _build_vederlag_section(
        ...
        if vederlag.status in ["ikke_relevant", "ikke_startet"]:
            elements.append(
                Paragraph(
                    f"<i>{self._format_status(vederlag.status)}</i>",
                    self.styles["KoeBodyText"],
                )
            )
            return elements
        ...
        if vederlag.bh_resultat:
            ...
            bh_data = []
            resultat_label = self.VEDERLAG_RESULTAT_MAP.get(
                vederlag.bh_resultat, vederlag.bh_resultat
            )
            bh_data.append(["Resultat:", resultat_label])

            if vederlag.godkjent_belop is not None:
                bh_data.append(
                    ["Godkjent beløp:", self._format_currency(vederlag.godkjent_belop)]
                )
    ...
    def _build_frist_section(self, state: SakState, events: list | None = None) -> list:
        ...
        if frist.status in ["ikke_relevant", "ikke_startet"]:
            elements.append(
                Paragraph(
                    f"<i>{self._format_status(frist.status)}</i>",
                    self.styles["KoeBodyText"],
                )
            )
            return elements
        ...
        if hasattr(frist, "varsel_type") and frist.varsel_type:
            varsel_label = self.FRIST_VARSEL_TYPE_MAP.get(
                frist.varsel_type, frist.varsel_type
            )
        ...
        if frist.bh_resultat:
            ...
            bh_data = []
            resultat_label = self.FRIST_RESULTAT_MAP.get(
                frist.bh_resultat, frist.bh_resultat
            )
            bh_data.append(["Resultat:", resultat_label])
    ...
    def _build_forsering_section(self, state: SakState) -> list:
        ...
        if not fd:
            elements.append(
                Paragraph("<i>Ingen forseringsdata</i>", self.styles["KoeBodyText"])
            )
            return elements
        ...
        elements.append(
            Paragraph(
                "1. Beregningsgrunnlag (30%-regelen, §33.8)",
                self.styles["KoeSectionHeader"],
            )
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (-1, -2),
                        0.5,
                        colors.HexColor(self.COLORS["border"]),
                    ),
                ]
            )
        )
        ...
        elements.append(
            Paragraph(
                "2. Entreprenør varsler forsering", self.styles["KoeSectionHeader"]
            )
        )
        elements.append(
            Paragraph("<b>ENTREPRENØR KREVER:</b>", self.styles["KoeSubHeader"])
        ...
        if fd.bh_aksepterer_forsering is not None:
            ...
            if fd.bh_godkjent_kostnad is not None:
                bh_data.append(
                    ["Godkjent kostnad:", self._format_currency(fd.bh_godkjent_kostnad)]
                )
    ...
    def _build_endringsordre_section(self, state: SakState) -> list:
        ...
        if not eo:
            elements.append(
                Paragraph("<i>Ingen endringsordredata</i>", self.styles["KoeBodyText"])
            )
            return elements
        ...
        elements.append(
            Paragraph(
                f"1. Endringsordre {eo.eo_nummer} (§31.3)",
                self.styles["KoeSectionHeader"],
            )
        )
        elements.append(
            Paragraph("<b>BYGGHERRE UTSTEDER:</b>", self.styles["KoeSubHeader"])
        )
        ...
        if eo.dato_utstedt:
            bh_data.append(
                ["Dato utstedt:", self._format_date_norwegian(eo.dato_utstedt)]
            )
        ...
        bh_data.append(
            ["Konsekvenser:", ", ".join(konsekvenser) if konsekvenser else "Ingen"]
        )

        # Beløp
        if eo.kompensasjon_belop is not None:
            bh_data.append(
                ["Kompensasjon:", self._format_currency(eo.kompensasjon_belop)]
            )
        ...
        if eo.oppgjorsform:
            ...
            bh_data.append(
                ["Oppgjørsform:", oppgjor_labels.get(eo.oppgjorsform, eo.oppgjorsform)]
            )
        ...
        if eo.te_akseptert is not None:
            elements.append(
                Paragraph("2. Entreprenør svarer", self.styles["KoeSectionHeader"])
            )
            elements.append(
                Paragraph("<b>ENTREPRENØR SVARER:</b>", self.styles["KoeSubHeader"])
            )
            ...
            if eo.dato_te_respons:
                te_data.append(
                    ["Dato respons:", self._format_date_norwegian(eo.dato_te_respons)]
                )
        ...
        # Status-linje
        elements.append(Spacer(1, 12))
        status_label = self._format_status(
    ...
    def _build_last_events(
        self, events: list | None, te_event_types: list[str], bh_event_types: list[str]
    ) -> list:
        ...
        if te_event or bh_event:
            ...
            if te_event:
                date = (
                    te_event.get("tidsstempel", "")[:10]
                    if te_event.get("tidsstempel")
                    else ""
                )
            ...
            if bh_event:
                date = (
                    bh_event.get("tidsstempel", "")[:10]
                    if bh_event.get("tidsstempel")
                    else ""
                )
    ...
    def _build_signature_section(
        self, saksbehandler: SignatureInfo, godkjenner: SignatureInfo
    ) -> list:
        ...
        elements.append(
            HRFlowable(
                width="100%", thickness=1, color=colors.HexColor(self.COLORS["border"])
            )
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 0),
                        (0, -1),
                        colors.HexColor(self.COLORS["gray_bg"]),
                    ),
                    (
                        "BACKGROUND",
                        (1, 0),
