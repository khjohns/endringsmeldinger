...
REQUIRED_TYPES = ["Krav om endringsordre", "Endringsordre", "Forsering"]
...
REQUIRED_CUSTOM_FIELDS = ["Byggherre", "Leverandør"]
...
TEST_DATA = {
    ...
    # FASE 3: BH Responses
    "bh_grunnlag": {"resultat": "godkjent", "begrunnelse": "Grunnlaget aksepteres"},
    "bh_vederlag": {
...
EO_TEST_DATA = {
    ...
    # TE aksept
    "te_aksept": {"akseptert": True, "kommentar": "Aksepterer endringsordre"},
}
...
class SetupValidator:
    ...
    def __init__(self):
        self.client: CatendaClient | None = None
    ...
    def validate_library_and_folder(self) -> bool:
        ...
        if self.library_id:
            ...
            if libraries:
                ...
                choice = input(
                    "Velg bibliotek (eller Enter for a hoppe over): "
                ).strip()
                if choice:
                    try:
                        idx = int(choice) - 1
    ...
    def validate_topic_board(self) -> bool:
        ...
        if board_with_fields:
            ...
            fields_map = {
                f.get("id"): f for f in board_with_fields.get("customFields", [])
            }
        ...
        if missing_types or missing_statuses:
            ...
            if confirm("Opprette manglende Types og Statuses automatisk?"):
                success = self._create_missing_board_config(
                    missing_types, missing_statuses
                )
                if not success:
                    return False
            else:
    ...
    def _create_missing_board_config(
        self, missing_types: list[str], missing_statuses: list[tuple[str, str]]
    ) -> bool:
    ...
    def validate_webhooks(self) -> bool:
        ...
        if not ngrok_url or not secret_path:
            ...
            for event in required_events:
                found = any(
                    wh.get("event") == event
                    and wh.get("target_url") == expected_url
                    and wh.get("state") == "ENABLED"
                    for wh in webhooks
                )
                if found:
...
class BaseTester:
    ...
    def _get_auth_headers(self, magic_token: str | None = None) -> dict[str, str]:
    ...
    def _create_case_directly(
        self,
        topic_guid: str,
        topic_title: str,
        sakstype: str = "koe",
        forsering_data: dict | None = None,
        ...
        try:
            ...
            if not byggherre:
                ...
                print_warn(
                    f"Byggherre custom field ikke satt, bruker fallback: {byggherre}"
                )
            if not leverandor:
                leverandor = TEST_DATA.get("leverandor")
                print_warn(
                    f"Leverandør custom field ikke satt, bruker fallback: {leverandor}"
                )
            ...
            base_url = (
                os.getenv("DEV_REACT_APP_URL")
                or os.getenv("REACT_APP_URL")
            ...
            if self.client.update_topic(
                topic_guid, topic_status=initial_catenda_status
            ):
                print_ok(f"Topic status satt til: {initial_catenda_status}")
            else:
                print_warn(
                    f"Kunne ikke sette topic status til: {initial_catenda_status}"
                )
    ...
    def _verify_catenda_status(
        self, topic_guid: str, expected_status: str, wait_seconds: int = 2
    ) -> bool:
        """
        ...
        if actual_status == expected_status:
            ...
            print_fail(
                f"Catenda status: {actual_status} (forventet: {expected_status})"
            )
            return False
    ...
    def _get_state_and_version(
        self, sak_id: str, magic_token: str
    ) -> tuple[dict | None, int]:
        """
...
class KOEFlowTester(BaseTester):
    ...
    def __init__(
        ...
        self.topic_guid: str | None = None
        self.sak_id: str | None = None
        self.topic_title: str | None = None
        self.magic_token: str | None = None

        # Event IDs for linking events
        self.grunnlag_event_id: str | None = None
        self.vederlag_event_id: str | None = None
        self.frist_event_id: str | None = None
    ...
    def _verify_new_comment_local(
        self, expected_increase: int = 1, wait_seconds: int = 3
    ) -> bool:
    ...
    def _verify_new_document_local(
        self, expected_increase: int = 1, wait_seconds: int = 3
    ) -> bool:
    ...
    def verify_webhook_received(self) -> bool:
        ...
        sak_id, magic_token = super()._create_case_directly(
            topic_guid=self.topic_guid, topic_title=self.topic_title, sakstype="koe"
        )
    ...
    def _fetch_sak_id_from_metadata(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{BACKEND_URL}/api/metadata/by-topic/{self.topic_guid}", timeout=5
            )
            if response.status_code == 200:
                data = response.json()
    ...
    def set_verification_baseline(self) -> bool:
        ...
        print_info(
            f"Baseline: {self.initial_comment_count} kommentar(er), {self.initial_document_count} dokument(er)"
        )
    ...
    def send_bh_responses(self) -> bool:
        ...
        # Hent gjeldende versjon
        state, current_version = self._get_state_and_version(
            self.sak_id, self.magic_token
        )
        if state is None:
        ...
        print_info(
            f"  Godkjent: {TEST_DATA['bh_vederlag']['total_godkjent_belop']:,.0f} kr av 150.000 kr"
        )
    ...
    def send_te_revisions(self) -> bool:
        ...
        print_info(
            f"  Nytt beløp: {TEST_DATA['vederlag_revisjon']['belop_direkte']:,.0f} kr"
        )
            ...
            vederlag_revision = {
                ...
                "event": {
                    ...
                    "data": {
                        ...
                        "belop_direkte": TEST_DATA["vederlag_revisjon"][
                            "belop_direkte"
                        ],
            ...
            frist_revision = {
                ...
                "event": {
                    ...
                    "data": {
                        ...
                        "varsel_type": frist_varsel_type
                        or "spesifisert",  # Bevar fra original
    ...
    def show_summary(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if vederlag:
                    belop = (
                        vederlag.get("belop_direkte")
                        or vederlag.get("kostnads_overslag")
                        or 0
                    )
        ...
        if self.client.delete_topic(self.topic_guid):
            ...
            try:
                ...
                # For Supabase-backend: slett events via API
                    ...
                    print_warn(
                        "Events ikke slettet - manuell opprydding i database kan være nødvendig"
                    )
...
class ForseringFlowTester(BaseTester):
    ...
    def __init__(
        ...
        self.koe_saker: list[
            dict[str, Any]
        ] = []  # [{sak_id, topic_guid, magic_token, frist_dager}]

        # Forsering-sak
        self.forsering_sak: dict[str, Any] | None = None
        self.forsering_topic_guid: str | None = None
    ...
    def create_koe_cases(self) -> bool:
        ...
        for i, koe_config in enumerate(FORSERING_TEST_DATA["koe_saker"], 1):
            ...
            sak_id, magic_token = self._create_case_directly(
                topic_guid=topic_guid, topic_title=koe_config["tittel"], sakstype="koe"
            )
            ...
            if success:
                print_ok(
                    f"Fristkrav sendt for KOE-{i}: {koe_config['frist']['antall_dager']} dager"
                )
            else:
    ...
    def validate_30_percent_rule(self) -> bool:
        ...
        if estimert_kostnad <= maks_forseringskostnad:
            print_ok(
                f"30%-regelen oppfylt: {estimert_kostnad:,.0f} <= {maks_forseringskostnad:,.0f}"
            )
            return True
        else:
            print_fail(
                f"30%-regelen IKKE oppfylt: {estimert_kostnad:,.0f} > {maks_forseringskostnad:,.0f}"
            )
            return False
    ...
    def create_forsering(self) -> bool:
        ...
        self.client.create_topic_relations(
            topic_id=self.forsering_topic_guid, related_topic_guids=koe_topic_guids
        )
        ...
        sak_id, magic_token = self._create_case_directly(
            ...
            forsering_data={
                ...
                "estimert_kostnad": FORSERING_TEST_DATA["forsering"][
                    "estimert_kostnad"
                ],
    ...
    def bh_respond_to_forsering(self) -> bool:
        ...
        success, _, _ = self._send_event(
            ...
            event_data={
                ...
                "godkjent_kostnad": FORSERING_TEST_DATA["bh_forsering_aksept"][
                    "godkjent_kostnad"
                ],
...
class EOFlowTester(BaseTester):
    ...
    def __init__(
        ...
        # Endringsordre-sak
        self.eo_sak: dict[str, Any] | None = None
        self.eo_topic_guid: str | None = None
    ...
    def create_koe_cases(self) -> bool:
        ...
        for i, koe_config in enumerate(EO_TEST_DATA["koe_saker"], 1):
            ...
            if success:
                print_ok(
                    f"Vederlagskrav sendt for KOE-{i}: {koe_config['vederlag']['belop_direkte']:,.0f} kr"
                )
            else:
    ...
    def create_endringsordre(self) -> bool:
        ...
        self.client.create_topic_relations(
            topic_id=self.eo_topic_guid, related_topic_guids=koe_topic_guids
        )
        ...
        for koe in self.koe_saker:
            self.client.create_topic_relations(
                topic_id=koe["topic_guid"], related_topic_guids=[self.eo_topic_guid]
            )
    ...
    def issue_endringsordre(self) -> bool:
        ...
        if success:
            ...
            print_info(
                f"  Kompensasjon: {eo_config['vederlag']['belop_direkte']:,.0f} kr"
            )
    ...
    def show_summary(self) -> None:
        ...
        if self.eo_sak:
            ...
            print(
                f"  BH kompensasjon: {eo_config['vederlag']['belop_direkte']:,.0f} kr"
            )
