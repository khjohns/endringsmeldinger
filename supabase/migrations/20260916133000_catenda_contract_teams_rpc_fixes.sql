-- Oppfølgingsmigrasjon for Catenda kontraktsteams og prosjektregistrering:
-- 1. koe_set_contract_teams: radlåsing (FOR UPDATE) og validering av at både BH og TE er representert
-- 2. koe_register_project: atomisk prosjektregistrering som delegerer til koe_set_contract_teams når p_teams IS NOT NULL

-- Oppdatering av koe_set_contract_teams med radlåsing og fullstendighetskontroll
...
BEGIN
    ...
    -- 4. Kontraktsteam dersom oppgitt: NULL betyr ingen teamendring.
    -- Når p_teams er oppgitt, delegeres innsettingen til koe_set_contract_teams()
    -- som sikrer radlåsing, validering av begge sider og atomisk feilhåndtering.
